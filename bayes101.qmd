...
filters:
  - shinylive
description: "Bayes rule, priors, and posteriors, plus why credible intervals answer the question decision-makers actually asked. Worked through declining user engagement and Karl Broman's socks."
share:
  permalink: "https://book.martinez.fyi/bayes101.html"
  description: "Bayes 101: priors, posteriors, and credible intervals for business decisions"
  linkedin: true
  email: true
  mastodon: true
---
...
simulate_socks_fast <- function(n_picked, n_sims = 100000, mu = 30, size = 4, shape1 = 15, shape2 = 2) {
  n_socks <- rnbinom(n_sims, mu = mu, size = size)
  prop_pairs <- rbeta(n_sims, shape1 = shape1, shape2 = shape2)
  n_pairs <- round(floor(n_socks / 2) * prop_pairs)
  n_odd <- n_socks - n_pairs * 2
  
  res_u <- integer(n_sims)
  res_p <- integer(n_sims)
  for (i in seq_len(n_sims)) {
    ns <- n_socks[i]
    np <- n_pairs[i]
    no <- n_odd[i]
    if (ns <= 0) next
    socks <- rep.int(seq_len(np + no), rep(c(2, 1), c(np, no)))
    picked <- sample(socks, size = min(n_picked, ns))
    tbl <- table(picked)
    res_u[i] <- sum(tbl == 1)
    res_p[i] <- sum(tbl == 2)
  }
...
# Run simulations
sock_sim <- simulate_socks_fast(n_picked = n_picked, n_sims = 100000)
...
simulate_socks_fast <- function(n_picked, n_sims, mu, size, shape1, shape2) {
  n_socks <- rnbinom(n_sims, mu = mu, size = size)
  prop_pairs <- rbeta(n_sims, shape1 = shape1, shape2 = shape2)
  n_pairs <- round(floor(n_socks / 2) * prop_pairs)
  n_odd <- n_socks - n_pairs * 2
...
ui <- fluidPage(
  ...
  titlePanel("Karl's Socks Simulation"),
  sidebarLayout(
    sidebarPanel(
      numericInput("seed", "Random Seed", value = 123, min = 1, step = 1),
      numericInput("n_sims", "Number of Simulations", value = 10000, min = 100, max = 50000, step = 500),
      sliderInput("mu", "mu", min = 15, max = 60, value = 40, step = 1),
      sliderInput("size", "Size", min = 3, max = 10, value = 4, step = 1),
      sliderInput("shape1", "Shape 1", min = 2, max = 20, value = 15, step = 1),
      sliderInput("shape2", "Shape 2", min = 2, max = 8, value = 2, step = 1),
      actionButton("run_sim", "Run Simulation", class = "btn-primary w-100")
    ),
    mainPanel(
      fluidRow(
        column(6, wellPanel(h4("Prior: Total Socks"), plotOutput("prior_socks", height = "280px"))),
        column(6, wellPanel(h4("Prior: Paired Proportion"), plotOutput("prior_prop", height = "280px")))
      ),
      fluidRow(
        column(6, wellPanel(h4("Prior and Posterior Distributions"), plotOutput("distribution_plot", height = "280px"))),
        column(6, wellPanel(h4("Joint Posterior Distribution"), plotOutput("joint_plot", height = "280px")))
      ),
      fluidRow(
        column(12, wellPanel(h4(textOutput("pr_pairs_text"))))
      )
    )
  )
)
...
server <- function(input, output) {
  ...
  sock_sim <- eventReactive(input$run_sim, ignoreNULL = FALSE, {
    set.seed(input$seed)
    withProgress(message = "Running sock simulation...", value = 0, {
      simulate_socks_fast(
        n_picked = 11,
        n_sims = input$n_sims,
        mu = input$mu,
  ...
  output$pr_pairs_text <- renderText({
    req(post_samples())
    if (nrow(post_samples()) == 0) return("Pr[# Pairs > 14] = N/A (No matching samples)")
    val <- mean(post_samples()$n_pairs > 14)
    paste0("Pr[# Pairs > 14] = ", scales::percent(val))
  })
  
  prior_data <- reactive({
    sock_sim() %>%
      count(n_socks) %>%
      mutate(prop = n / sum(n), type = "Prior")
  })
  ...
  posterior_data <- reactive({
    ...
    post_samples() %>%
      count(n_socks) %>%
      mutate(prop = n / sum(n), type = "Posterior")
  })
  
  output$prior_socks <- renderPlot({
    sock_counts <- data.frame(n_socks = 0:100)
    sock_counts$probability <- dnbinom(sock_counts$n_socks, mu = input$mu, size = input$size)
    ggplot(sock_counts, aes(x = n_socks, y = probability)) +
      geom_col(fill = "skyblue", color = "black") + 
      scale_y_continuous(labels = scales::percent) +
      theme_minimal() +
  ...
  output$prior_prop <- renderPlot({
    proportions <- seq(0, 1, length.out = 100)
    density_values <- dbeta(proportions, shape1 = input$shape1, shape2 = input$shape2)
    ggplot(data.frame(proportion = proportions, density = density_values),
           aes(x = proportion, y = density)) +
      geom_line(color = "darkgreen") +
      theme_minimal() +

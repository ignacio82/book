...
#| fig-alt: "Three panels. In each, a blue curve with a band tracks a black truth curve across exposures eleven to sixteen, with the band widening past the dotted line."
#| fig-width: 8.5
#| fig-height: 4
...
#| fig-alt: "Three lines rising with promotional intensity. The truth and the model given the covariate rise together; the model without it sits above both and rises more steeply."
#| fig-width: 8.5
#| fig-height: 4
...
Given the covariate, the model recovers both channels: the residual scale drops to its true value because promotions no longer look like noise, and the effect's dependence on promotional intensity comes out close to the truth. Without it, the residual absorbs the seller-week part of the promotion, and the calendar part goes somewhere worse. In a three-wave rollout, exposure and calendar time are nearly the same clock, so a promotional week that lifts every seller is read by the exposure trajectory as part of the treatment effect: the blind curve sits above the truth, and its slope in promotional intensity is nearly twice the true one, which is not moderation but the promotion calendar itself. A calendar split in the prognostic forest could absorb that common part, but not the moderation; the covariate handles both.

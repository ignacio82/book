...
description: "You cannot force users into a behavior, but you can encourage them. Randomized encouragement designs and instrumental variables, intention-to-treat versus complier effects, and the assumptions behind them."
share:
  permalink: "https://book.martinez.fyi/iv.html"
  description: "Randomized encouragement design: causal effects when you cannot randomize behavior"
  linkedin: true
  email: true
  mastodon: true
author:
...
each participant, we consider:
  ...
  - $Z$: The encouragement (Z = 1 if encouraged, Z = 0 if not encouraged)
  - $Y(1) = Y(Z = 1)$: The outcome if encouraged
  - $Y(0) = Y(Z = 0)$: The outcome if not encouraged
  - $D(1) = D(Z = 1)$: The treatment status if encouraged
  - $D(0) = D(Z = 0)$: The treatment status if not encouraged
...
2.  **Complier Average Causal Effect (CACE):** In other scenarios, our focus
    ...
    where $Y(1)$ and Y(0) are the outcomes under encouragement versus not, and $D(1)$
    and $D(0)$ are actual treatment received under encouragement versus not. For
    instance, a product team might be interested in the impact of actually using
    a new feature on user engagement, rather than just being encouraged to use it.

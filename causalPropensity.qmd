...
# ── libraries ────────────────────────────────────────────────────────────────
library(posterior)   # ess_bulk(), ess_tail(), as_draws_matrix()
library(bayesplot)

# Posterior draws of the ATE  (vector length = n_draws)
ate_draws <- colMeans(bcf_fit$tau_hat_train)

# ── 1. trace plot (single chain) ─────────────────────────────────────────────
...
draws_mat    <- posterior::as_draws_matrix(matrix(ate_draws, ncol = 1))
ate_ess_bulk <- posterior::ess_bulk(draws_mat)
ate_ess_tail <- posterior::ess_tail(draws_mat)

cat(sprintf(
  "Bulk ESS : %.0f\nTail ESS : %.0f\n",
  ate_ess_bulk, ate_ess_tail
))

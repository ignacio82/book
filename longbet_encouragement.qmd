...
Only the ready-and-scaled segment is worth encouraging at this cost, and the ready-but-small segment complies just as eagerly. With `r sum(true_cum_test > cost)` profitable accounts and capacity for `r capacity`, the constraint binds.

```{r fig-adoption}
#| fig-cap: "Adoption by assigned arm in the pilot. The offered arm jumps at week 4; the control arm keeps adopting organically, so the gap stops growing after week 5 while exposure keeps accumulating."
#| fig-alt: "Two rising curves over eight weeks with a shaded gap between them that opens at week four and then holds roughly constant."
#| fig-width: 8
#| fig-height: 4
...
#| fig-alt: "Three panels, one per segment. In the ready-and-scaled panel the truth rises steeply and only the adoption-clock model follows it; every other estimator flattens out well below. In the other two panels the truth stays near zero, the adoption-clock model tracks it, and the rest sit above it."
#| fig-width: 11
#| fig-height: 4.2
traj_df <- bind_rows(

---
title: "Matching"
description: "Building a valid comparison group by pairing treated units with similar untreated ones, the propensity score paradox, and a clear-eyed account of what matching cannot fix."
share:
  permalink: "https://book.martinez.fyi/matching.html"
  description: "Matching: constructing comparison groups from observational data, and its limits"
  linkedin: true
  email: true
  mastodon: true
author:
...
## Target Trials and Comparison Groups

When randomized assignment is off the table, constructing a comparison group is not simply an exercise in running a matching algorithm on whatever data happens to be stored in a warehouse. As @hernan2016using emphasize, causal analysis of observational data is most transparently organized as the **emulation of a hypothetical target trial**---the pragmatic randomized experiment we would have conducted to answer the causal question if we had been able to.

In the target trial framework, matching is just one adjustment method within a broader protocol, not another name for that framework or a way to recreate randomization. Emulating a target trial requires explicitly defining seven key protocol components before touching the analysis: eligibility criteria, treatment strategies, assignment procedure, time zero and follow-up window, outcome of interest, causal estimand, and statistical analysis plan.

@tbl-target-trial-bootcamp illustrates how these protocol elements apply to our AI bootcamp example. These protocol details are illustrative; the cross-sectional simulation that follows demonstrates confounding adjustment and its limits, rather than a full longitudinal target trial emulation.

| Protocol element | Target randomized trial | Observational emulation (Bootcamp) |
| :--- | :--- | :--- |
| **Eligibility criteria** | Full-time software engineers at TechGiant Inc. with active employment. | Same criteria applied to HR records; excludes contract or part-time staff. |
| **Treatment strategies** | 1. Attend and complete the AI bootcamp.<br>2. Continue normal engineering work without bootcamp. | 1. Initiated bootcamp participation within enrollment window.<br>2. Did not initiate bootcamp participation. |
| **Assignment procedure** | Random assignment (1:1) at time zero. | Observational self-selection; unconfoundedness conditional on observed covariates assumed. |
| **Time zero & follow-up** | Time zero is day of randomization. Follow-up is 12 months post-randomization. | Time zero is cohort announcement date. Follow-up is 12 months post-announcement. |
| **Outcome** | Salary increase ($) over the 12-month follow-up period. | Salary increase ($) recorded in annual compensation review. |
| **Causal estimand** | Average treatment effect (ATE) in the eligible engineer population. | Average treatment effect on the treated (ATT) or effect in the matched subpopulation with common overlap. |
| **Analysis plan** | Intention-to-treat comparison of mean salary increases across randomized arms. | Nearest-neighbor matching on pre-treatment covariates with balance diagnostics and regression adjustment. |
: Target trial protocol elements applied to the AI bootcamp evaluation {#tbl-target-trial-bootcamp}

A fundamental principle of this framework is temporal alignment: **covariates used for matching must precede treatment** (measured prior to time zero). Matching on post-treatment variables can introduce collider bias or inadvertently control away part of the treatment's effect. Furthermore, achieving observed balance across measured covariates does not prove the absence of unmeasured confounding. Finally, exclusions for lack of overlap necessarily change the target population from the full eligible cohort to the matched subset; when units without common support are trimmed, the estimand shifts from an overall population average to the specific subpopulation for which comparable matches exist.
...
3.  Balance Checking: Regardless of the matching method used, researchers
    should always assess covariate balance and overlap directly across the
    original covariates before and after matching, rather than relying solely
    on the distribution of a fitted propensity score.
...
# Estimate ATT on the matched sample (1:1 matching pairs each treated participant)
matched_model <- lm(salary_increase ~ bootcamp,
                    data = matched_data,
                    weights = weights)
matched_att <- coef(matched_model)["bootcamp"]

# Print results
cat("True ATE / ATT: $2000\n")
cat("Naive comparison estimate:", round(naive_ate, 2), "\n")
cat("Matched estimate (ATT in matched sample):", round(matched_att, 2), "\n")
...
and understanding of those who wield it.

In later chapters on stochastic tree ensembles, we will see that propensity
information plays an essential role even in flexible Bayesian modeling. As
discussed in [Bayesian Causal Forests](bcf.qmd), incorporating the estimated
propensity score into the prognostic forest helps mitigate *regularization-induced
confounding* (RIC) under the model's assumptions. However, neither matching nor
conditioning on a propensity score eliminates unmeasured confounding: if key
covariates determining treatment assignment remain unobserved, observational
estimates remain vulnerable to hidden bias.

::: {.callout-tip}
## Learn more
 - @hernan2016using Using Big Data to Emulate a Target Trial When a Randomized Trial Is Not Available.

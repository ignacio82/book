...
$$
n^* = \frac{\sqrt{N}}{4}\left[\left(\frac{s}{\sigma}\right)^2 + \left(\frac{3}{4}\left(\frac{s}{\sigma}\right)^2\right)^2\right]
$$
...
A notable advantage of the test & roll approach is its management of
"regret"—the lost profit compared to the ideal scenario of perfect
information. The expected regret under profit-maximizing test & roll is
approximately $O(\sqrt{N})$, significantly lower than conventional hypothesis
testing, which can incur regret on the order of $\Omega(N)$.

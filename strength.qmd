...
Traditional power calculations focus on controlling Type I and Type II errors
and are designed to detect statistically significant differences. In business
experiments, however, the ultimate goal is not merely to "reject a null
hypothesis" but to maximize profit by deploying the better treatment to the
largest possible segment of your audience. The test & roll framework re-frames
sample size determination as an explicit trade-off between the learning cost
incurred during the test phase and the opportunity cost of deploying a
suboptimal treatment in the roll phase [see @mcdonnell2018test].

### Balancing Learning and Opportunity Cost

In a conventional A/B test, you might allocate a large portion of your sample to
achieve high statistical power. Yet doing so means that more customers are
exposed to a treatment that may later be deemed inferior. In contrast, the
profit-maximizing approach weighs:
...
By explicitly quantifying these trade-offs, the optimal (or profit‐maximizing)
test sample size is often substantially smaller than the sample size suggested
by traditional hypothesis testing.

### A Closed-Form Insight

For the common case with symmetric Normal priors (i.e. when you assume no a
priori difference between treatments), a closed-form expression for the
profit‐maximizing sample size can be derived. Without delving into all the
mathematical details, the key takeaway is that the optimal test size, ( n^\* ),
grows sub-linearly with the total population ( N ) and is sensitive to both the
response noise ( s ) and the uncertainty in the treatment effect ( \\sigma ). In
simplified terms, one finds that

\[ n^\* \\propto \sqrt{N} \\quad \text{and} \\quad n^\* \\sim
O\!\\left(\frac{s}{\\sigma}\\right), \]

which contrasts sharply with the linear (or even super-linear) growth in sample
size dictated by traditional power analysis. This means that for noisy responses
or when the total available population is small, a much smaller test is not only
feasible but also profit-maximizing.

### Regret and Decision Quality

Another appealing feature of the test & roll approach is its behavior in terms
of regret—the loss in profit relative to an ideal scenario of perfect
information. Analyses show that the expected regret from a profit-maximizing
test & roll scales as ( O(\sqrt{N}) ). By comparison, conventional hypothesis
tests often incur regret on the order of ( \\Omega(N) ) when sample sizes are
oversized relative to the finite population. In other words, a carefully sized
test can nearly match the performance of more dynamic algorithms (such as those
based on multi-armed bandits) while maintaining operational simplicity and
transparency.
...
Incorporating the profit-maximizing test & roll framework shifts the focus from
statistical significance to business impact. By balancing the cost of testing
against the opportunity cost of deploying a suboptimal treatment, this
decision-centric approach offers a compelling alternative to traditional power
analysis, especially in contexts with noisy responses or limited populations.
...
The plots above show that our prior distributions for the outcome (churn) and the impact of the intervention are reasonable. In particular our prior before seeing the outcome data is that the probability that the intervention has a meaningful impact is `r scales::percent(prior$prob_above_threshold)`.
...
In this case, the probability of making the right choice (rolling out the new
feature when it has no meaningful impact on churn) is
`r scales::percent(mean(simulations < 0.75))`.

::: {.callout-tip}
## Learn more
  - @dong2013powerup PowerUp!: A tool for calculating minimum detectable effect
    sizes and minimum required sample sizes for experimental and
    quasi-experimental design studies.

  - @bloom2008core The core analytics of randomized experiments for social
    research.
    
  - @gelman2014beyond Beyond power calculations: Assessing type S (sign) and
